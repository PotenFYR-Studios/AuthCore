...
public class Discord {
  ...
  private static int linkCommand(ServerCommandSource source) {
    try {
      ...
      return AuthCoreServer.LOGGER.toUser(
          1, player.networkHandler, AuthCoreServer.messages.promptUserDiscordLinkCode, code);
    } catch (Exception err) {
  ...
  private static int unlinkCommand(ServerCommandSource source) {
    try {
      ServerPlayerEntity player = source.getPlayer();
      if (player == null)
        return AuthCoreServer.LOGGER.info(0, "This command can't be executed from console!");
      ...
      if (user.discordId == null || user.discordId.isEmpty())
        return AuthCoreServer.LOGGER.toUser(
            1, player.networkHandler, AuthCoreServer.messages.promptUserDiscordNotLinked);
      ...
      return AuthCoreServer.LOGGER.toUser(
          1, player.networkHandler, AuthCoreServer.messages.promptUserDiscordUnlinked);
    } catch (Exception err) {
