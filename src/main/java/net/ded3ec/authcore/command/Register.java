...
public class Register {
    ...
    public static void load(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal("register").requires((ctx) -> {
            ...
            return user != null && user.isInLobby.get() && !user.isAuthenticated.get() && !user.isRegistered.get() && Permissions.check(ctx.getPlayer(), AuthCore.config.commands.user.register.luckPermsNode, PermissionLevel.fromLevel(AuthCore.config.commands.user.register.permissionsLevel));
        }).then(argument("password", string()).executes(ctx -> registerCommand(ctx.getSource(), getString(ctx, "password"), null)).then(argument("confirm-password", string()).requires(ctx -> AuthCore.config.session.authentication.registerPasswordConfirmation).executes(ctx -> registerCommand(ctx.getSource(), getString(ctx, "password"), getString(ctx, "confirm-password"))))));
    }
     ...
     */
    private static int registerCommand(ServerCommandSource source, @NotNull String password, @Nullable String confirmPassword) {
        try {
            ...
            if (user.isRegistered.get())
                return Logger.toUser(0, player.networkHandler, AuthCore.messages.promptUserAlreadyRegistered);
     ...
     */
    private static boolean checkPassword(@NotNull ServerPlayerEntity player, @NotNull String password, @Nullable String confirmPassword) {
        if (StringUtils.isBlank(password))
            return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserPasswordIsBlank);
        else if (AuthCore.config.session.authentication.registerPasswordConfirmation && StringUtils.isBlank(confirmPassword))
            return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserConfirmPasswordIsBlank);
        else if (AuthCore.config.session.authentication.registerPasswordConfirmation && !password.equals(confirmPassword))
            return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserPasswordDoesNotMatch);
