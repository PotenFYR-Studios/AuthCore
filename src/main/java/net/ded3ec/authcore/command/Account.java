...
public class Account {
    ...
    public static void load(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal("account").then(literal("logout").requires((ctx) -> {
            if (ctx.getPlayer() == null) return false;
            UUID uuid = ctx.getPlayer().getUuid();
            String username = ctx.getPlayer().getName().getString();
            User user = User.getUser(username, uuid);

            return user != null && user.isActive && Permissions.check(ctx.getPlayer(), AuthCore.config.commands.user.logout.luckPermsNode, PermissionLevel.fromLevel(AuthCore.config.commands.user.logout.permissionsLevel));
        }).executes(ctx -> logoutCommand(ctx.getSource()))).then(literal("unregister").requires((ctx) -> {
            if (ctx.getPlayer() == null) return false;
            UUID uuid = ctx.getPlayer().getUuid();
            String username = ctx.getPlayer().getName().getString();
            User user = User.getUser(username, uuid);

            return user != null && user.isAuthenticated.get() && user.isRegistered.get() && Permissions.check(ctx.getPlayer(), AuthCore.config.commands.user.unregister.luckPermsNode, PermissionLevel.fromLevel(AuthCore.config.commands.user.unregister.permissionsLevel));
        }).executes(ctx -> unregisterCommand(ctx.getSource()))).then(literal("set-password").then(argument("new-password", StringArgumentType.string()).requires((ctx) -> {
            if (ctx.getPlayer() == null) return false;
            UUID uuid = ctx.getPlayer().getUuid();
            String username = ctx.getPlayer().getName().getString();
            User user = User.getUser(username, uuid);

            return user != null && user.isRegistered.get() && user.isAuthenticated.get() && Permissions.check(ctx.getPlayer(), AuthCore.config.commands.user.changepassword.luckPermsNode, PermissionLevel.fromLevel(AuthCore.config.commands.user.changepassword.permissionsLevel));
        }).executes(ctx -> setPasswordCommand(ctx.getSource(), getString(ctx, "new-password"))))));
    }
    ...
    private static int logoutCommand(ServerCommandSource source) {
        try {
            ...
            Logger.debug(0, "{} used '/account logout' command in the Server!", player.getName().getString());

            UUID uuid = player.getUuid();
            String username = player.getName().getString();
            User user = User.getUser(username, uuid);

            if (user == null) return Logger.toKick(0, player.networkHandler, AuthCore.messages.promptUserNotFoundData);
    ...
    private static int setPasswordCommand(ServerCommandSource source, String password) {
        try {
            ...
            Logger.debug(1, "{} used '/account set-password <new-password>' command in the Server!", player.getName().getString());
            ...
            if (checkNewPassword(player, tempPassword, user.password)) {
                ...
                return Logger.toUser(1, player.networkHandler, AuthCore.messages.promptUserPasswordChangedSuccessfully);

            } else return 0;
        } catch (Exception err) {
            return Logger.error(0, "Faced Error in '/account set-password <new-password>' Command: ", err);
        }
    }
    ...
    private static int unregisterCommand(ServerCommandSource source) {
        try {
            ...
            Logger.debug(1, "{} used '/account unregister' command in the Server!", player.getName().getString());
            ...
            if (user == null)
                return Logger.toUser(0, player.networkHandler, AuthCore.messages.promptAdminUserNotFound, uniqueId);
            else if (!user.isRegistered.get())
                return Logger.toUser(0, player.networkHandler, AuthCore.messages.promptUserNotRegistered);

            user.delete("Unregistered by the User", false);

            return Logger.toKick(1, source.getPlayer().networkHandler, AuthCore.messages.promptUserUnRegisteredSuccessfully);
     ...
     */
    private static boolean checkNewPassword(@NotNull ServerPlayerEntity player, @NotNull String newPassword, @NotNull String oldPassword) {
        if (StringUtils.isBlank(newPassword))
            return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserPasswordIsBlank);
        else if (!AuthCore.config.passwordRules.allowReuse && newPassword.equals(oldPassword))
            return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserDuplicatePassword);
