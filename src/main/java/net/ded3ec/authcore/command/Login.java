...
public class Login {
    ...
    public static void load(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal("login").requires((ctx) -> {
            ...
            return user != null && user.isInLobby.get() && !user.isAuthenticated.get() && user.isRegistered.get() && Permissions.check(ctx.getPlayer(), AuthCore.config.commands.user.login.luckPermsNode, PermissionLevel.fromLevel(AuthCore.config.commands.user.login.permissionsLevel));
        }).then(
                // Add the "password" argument to the command.
                argument("password", string()).executes(ctx -> execute(ctx.getSource(), getString(ctx, "password")))));
    }
    ...
    private static int execute(ServerCommandSource source, @NotNull String password) {
        try {
            ...
            if (user == null) return Logger.toUser(0, player.networkHandler, AuthCore.messages.promptUserNotFoundData);
            else ++user.loginAttempts;

            // Check if the user has exceeded the maximum login attempts.
            if (user.loginAttempts >= AuthCore.config.session.authentication.maxLoginAttempts)
                return Logger.toKick(0, player.networkHandler, AuthCore.messages.promptUserExceededLoginAttempts, AuthCore.config.session.authentication.maxLoginAttempts, Misc.TimeConverter.toDuration(AuthCore.config.session.cooldownAfterKickMs));
                ...
                return Logger.toUser(1, player.networkHandler, AuthCore.messages.promptUserLoggedInSuccessfully);
            } else
