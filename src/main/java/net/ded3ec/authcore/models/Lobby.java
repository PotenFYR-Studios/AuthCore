...
public class Lobby {
    ...
    public void handleTeleport() {
        ...
        if (AuthCore.config.lobby.limboConfig.enabled && (!this.user.isRegistered.get() || !AuthCore.config.lobby.limboConfig.onlyOnFirstTime)) {
            ...
            if (this.position == null) {
                blockPos = BlockPos.ofFloored(AuthCore.config.lobby.limboConfig.location.x, AuthCore.config.lobby.limboConfig.location.y, AuthCore.config.lobby.limboConfig.location.z);
    ...
    public void unlock() {
        ...
        Logger.debug(false, "{} has been taken out from the lobby/restricted mode!", this.user.username);
    ...
    private void handleTimeout() {
        ...
        if (user.handler.getLatency() >= 600) loginTimeoutMs = AuthCore.config.lobby.timeout.timeoutAbove600LatencyMs;
        ...
        if (loginTimeoutMs > 0) this.lobbyTimeoutTask = Misc.TaskScheduler.getInstance().setTimeout(() -> {
            if (user.isActive && this.user.isInLobby.get())
                Logger.toKick(false, this.user.handler, AuthCore.messages.promptUserAuthenticationExpiredTimeout, Misc.TimeConverter.toDuration(_loginTimeoutMs));
        }, loginTimeoutMs);

        long endIntervalMs = System.currentTimeMillis() + loginTimeoutMs;

        if (AuthCore.config.session.messageReminderIntervalMs > 0)
            if (!user.isRegistered.get()) this.lobbyIntervalTask = Misc.TaskScheduler.getInstance().setInterval(() -> {
                if (this.user.isActive && this.user.isInLobby.get())
                    Logger.toUser(true, this.user.handler, AuthCore.messages.promptUserRegisterCommandReminderInterval, _loginTimeoutMs > 0 ? Misc.TimeConverter.toDuration(endIntervalMs - System.currentTimeMillis()) : "Infinite");
            }, AuthCore.config.session.messageReminderIntervalMs);
            else this.lobbyIntervalTask = Misc.TaskScheduler.getInstance().setInterval(() -> {
                if (this.user.isActive && this.user.isInLobby.get())
                    Logger.toUser(true, this.user.handler, AuthCore.messages.promptUserLoginCommandReminderInterval, _loginTimeoutMs > 0 ? Misc.TimeConverter.toDuration(endIntervalMs - System.currentTimeMillis()) : "Infinite");
            }, AuthCore.config.session.messageReminderIntervalMs);
    }
    ...
    public static class Snapshot {
        ...
        public Snapshot(ServerPlayerEntity player) {
            ...
            if (AuthCore.config.lobby.invisibleUnauthorized)
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.INVISIBILITY, Integer.MAX_VALUE, 1, false, false), null);

            if (AuthCore.config.lobby.applyBlindnessEffect)
                player.addStatusEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, Integer.MAX_VALUE, 1, false, false), null);
        ...
        public void reset(ServerPlayerEntity player) {
            ...
            if (this.operator)
                server.getPlayerManager().addToOperators(player.getPlayerConfigEntry(), Optional.ofNullable(this.permissions), Optional.of(true));
        }
        ...
        private void teleport(ServerPlayerEntity player, BlockPos pos, ServerWorld world) {
            ...
            if (server != null && world != null)
                player.teleport(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, EnumSet.noneOf(PositionFlag.class), player.getYaw(), player.getPitch(), true);
        }
        ...
        private BlockPos getTeleportPos(ServerPlayerEntity player, BlockPos pos, ServerWorld world) {
            ...
            boolean playerIsAirborne = player.getAbilities().flying || player.isGliding() || player.hasVehicle() || !player.isOnGround();

            boolean playerInGap = (player.isGliding() || !player.isOnGround()) && !world.getBlockState(pos).isAir() && world.getBlockState(pos.down()).isSolidBlock(world, pos.down()) && world.getBlockState(pos.up()).isSolidBlock(world, pos.up());

            boolean playerIsInWater = player.isSwimming() || player.isTouchingWater() || (player.getPose() == EntityPose.SWIMMING);
        ...
        private BlockPos findWaterSurface(ServerWorld world, BlockPos origin) {
            ...
            while (check.getY() <= topY && world.getBlockState(check).isOf(Blocks.WATER)) check.move(0, 1, 0);
        ...
        private boolean isBlockSafe(ServerWorld world, BlockPos pos) {
            ...
            boolean stableBlockState = state.isSolidBlock(world, pos) || state.getBlock() instanceof LeavesBlock || state.getBlock() instanceof CarpetBlock || state.getBlock() instanceof ScaffoldingBlock || state.getBlock() instanceof SlimeBlock || state.getBlock() instanceof HoneyBlock || state.getBlock() instanceof FenceBlock || state.getBlock() instanceof WallBlock || state.getBlock() instanceof LadderBlock || state.getBlock() instanceof VineBlock || state.getBlock() instanceof BigDripleafBlock || state.getBlock() instanceof SmallDripleafBlock;

            return (state.isSolidBlock(world, pos) || stableBlockState || !state.getCollisionShape(world, pos).isEmpty() || world.getBlockState(pos).getBlock() == Blocks.SNOW || world.getBlockState(pos).getBlock() == Blocks.SNOW_BLOCK) && world.getBlockState(pos.up()).isAir() && world.getBlockState(pos.up(2)).isAir();
        }
    }
}
