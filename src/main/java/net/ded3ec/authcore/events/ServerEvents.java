...
public class ServerEvents {
    ...
    public static void onPlayerJoin(ServerPlayNetworkHandler handler, PacketSender sender, MinecraftServer server) {
        ...
        if (user != null && user.isActive) {
            if (user.isInLobby.get() && AuthCore.config.session.authentication.blockDuplicateRegister) {
                ...
                return;
            } else if (user.isAuthenticated.get() && AuthCore.config.session.authentication.blockDuplicateSession) {
                Logger.toKick(false, handler, AuthCore.messages.promptUserAnotherAccountSession);
                return;
            }
        } else if (user == null)
            user = new User(handler.player.getUuid(), handler.player.getName().getString(), System.currentTimeMillis(), handler.player.getName().getString().equals(Misc.getPremiumUsername(handler.player.getUuid())));
        ...
        else if (AuthCore.config.session.sessionFromSameIPOnly && user.isRegistered.get() && (user.ipAddress != null) && (!user.ipAddress.equals(handler.player.getIp())))
            Logger.toKick(false, handler, AuthCore.messages.promptUserDifferentIpLoginNotAllowed);
        else if (!AuthCore.config.session.authentication.allowCrackedPremiumNames && Misc.getPreimumUuid(user.username) != null && Misc.getPremiumUsername(handler.getPlayer().getUuid()) == null)
            user.kick(AuthCore.messages.promptUserPremiumNameNotAllowed);
        else if (user.uuid.equals(handler.player.getUuid()) && (user.ipAddress != null) && (user.ipAddress.equals(handler.player.getIp())) && user.isActiveSession.get()) {
        ...
        } else if (user.uuid.equals(handler.player.getUuid()) && AuthCore.config.session.authentication.premiumAutoLogin && user.isPremium) {
        ...
        } else if (!user.uuid.equals(handler.player.getUuid()) && user.isPremium && AuthCore.config.session.authentication.premiumAutoLogin)
            user.kick(AuthCore.messages.promptUserPremiumDifferentUUID);
        else if (user.lastKickedMs > 0 && (AuthCore.config.session.cooldownAfterKickMs > (System.currentTimeMillis() - user.lastKickedMs)))
            user.kick(AuthCore.messages.promptUserCooldownAfterKickNotExpired, Misc.TimeConverter.toDuration((System.currentTimeMillis() - user.lastKickedMs) - AuthCore.config.session.cooldownAfterKickMs));
        else if (AuthCore.config.lobby.maxLobbyUsers > 0 && AuthCore.config.lobby.maxLobbyUsers <= Lobby.users.size())
    ...
    public static void onPlayerLeave(ServerPlayNetworkHandler handler, MinecraftServer server) {
        ...
        if (user != null) {
            ...
            user.isInCombactPenalty = user.lastCombactDetectMs > 0 && AuthCore.config.session.combatTimeout > 0 && ((System.currentTimeMillis() - user.lastCombactDetectMs) < AuthCore.config.session.combatTimeout);
     ...
     */
    public static boolean onAllowChatMessage(SignedMessage message, ServerPlayerEntity player, MessageType.Parameters parameters) {
