...
public class EntityEvents {
     ...
     */
    public static ActionResult onEntityAttack(PlayerEntity player, World world, Hand hand, Entity entity, @Nullable EntityHitResult entityHitResult) {
        ...
        if (user != null && user.isInLobby.get()) {
            ...
            if (entity instanceof PlayerEntity && !AuthCore.config.lobby.allowAttackingPlayer)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserAttackPlayerNotAllowed);
            ...
            if (entity instanceof HostileEntity && !AuthCore.config.lobby.allowAttackingHostileMobs)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserAttackHostileMobsNotAllowed);

            // Prevent attacking animals if disallowed
            if (entity instanceof AnimalEntity && !AuthCore.config.lobby.allowAttackingAnimals)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserAttackAnimalNotAllowed);
            ...
            if (entity instanceof PassiveEntity && !AuthCore.config.lobby.allowAttackingFriendlyMobs)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserAttackFriendlyMobsNotAllowed);

            // Prevent attacking neutral mobs if disallowed
            if (entity instanceof MobEntity && !(entity instanceof AnimalEntity) && !AuthCore.config.lobby.allowAttackNeutralMobs)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserAttackNeutralMobsNotAllowed);

            // Prevent attacking mountable entities if disallowed
            if (entity instanceof BoatEntity || entity instanceof MinecartEntity || entity instanceof AbstractHorseEntity || entity instanceof CamelEntity || entity instanceof PigEntity || entity instanceof StriderEntity && !AuthCore.config.lobby.allowAttackMountableEntity)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractMountableEntityNotAllowed);

            // Prevent attacking any entity if disallowed
            if (entity instanceof Entity && !AuthCore.config.lobby.allowAttackEntity)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractEntityNotAllowed);
     ...
     * otherwise ActionResult.PASS.
     */
    public static ActionResult onEntityUse(PlayerEntity player, World world, Hand hand, Entity entity, @Nullable EntityHitResult entityHitResult) {
        UUID uuid = player.getUuid();
        String username = player.getName().getString();
        User user = User.getUser(username, uuid);
        ...
        if (user != null && user.isInLobby.get()) {
            ...
            if (entity instanceof PlayerEntity && !AuthCore.config.lobby.allowPlayerInteractWith)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractPlayersNotAllowed);
            ...
            if (entity instanceof HostileEntity && !AuthCore.config.lobby.allowHostileMobsInteractWith)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractHostileMobsNotAllowed);
            ...
            if (entity instanceof AnimalEntity && !AuthCore.config.lobby.allowAnimalInteractWith)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractAnimalsNotAllowed);
            ...
            if (entity instanceof PassiveEntity && !AuthCore.config.lobby.allowFriendlyMobsInteractWith)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractFriendlyMobsNotAllowed);

            // Prevent interacting with neutral mobs if disallowed
            if (entity instanceof MobEntity && !(entity instanceof AnimalEntity) && !AuthCore.config.lobby.allowNeutralMobsInteractWith)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractNeutralMobsNotAllowed);

            // Prevent interacting with mountable entities if disallowed
            if (entity instanceof BoatEntity || entity instanceof MinecartEntity || entity instanceof AbstractHorseEntity || entity instanceof CamelEntity || entity instanceof PigEntity || entity instanceof StriderEntity && !AuthCore.config.lobby.allowMountableInteractWith)
                return Logger.toUser(ActionResult.FAIL, user.handler, AuthCore.messages.promptUserInteractMountableEntityNotAllowed);
    ...
    public static boolean onEntityDamage(LivingEntity entity, DamageSource damageSource, float v) {
        ...
        if (user != null && user.isInLobby.get()) {
            ...
            if (damageSource.getAttacker() instanceof MobEntity) return AuthCore.config.lobby.allowMobDamage;

                // Prevent damage from players if disallowed
            else if (AuthCore.config.lobby.preventPlayerDamage && (damageSource.getAttacker() instanceof ServerPlayerEntity))
                return Logger.toUser(false, ((ServerPlayerEntity) damageSource.getAttacker()).networkHandler, AuthCore.messages.promptUserAttackLobbyUserNotAllowed);
