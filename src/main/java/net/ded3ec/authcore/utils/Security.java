...
public class Security {
    ...
    public static class Password {
        ...
        public static boolean check(ServerPlayerEntity player, String password) {
            ...
            if (AuthCore.config.passwordRules.upperCase.enabled && (uppercaseCount < AuthCore.config.passwordRules.upperCase.min || uppercaseCount > AuthCore.config.passwordRules.upperCase.max))
                return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserUpperCaseNotPresent, AuthCore.config.passwordRules.upperCase.min, AuthCore.config.passwordRules.upperCase.max);

            // Check lowercase letter requirements if the rule is enabled
            if (AuthCore.config.passwordRules.lowerCase.enabled && (lowercaseCount < AuthCore.config.passwordRules.lowerCase.min || lowercaseCount > AuthCore.config.passwordRules.lowerCase.max))
                return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserLowerCaseNotPresent, AuthCore.config.passwordRules.lowerCase.min, AuthCore.config.passwordRules.lowerCase.max);

            // Check digit requirements if the rule is enabled
            if (AuthCore.config.passwordRules.digits.enabled && (digitsCount < AuthCore.config.passwordRules.digits.min || digitsCount > AuthCore.config.passwordRules.digits.max))
                return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserDigitNotPresent, AuthCore.config.passwordRules.digits.min, AuthCore.config.passwordRules.digits.max);

            // Check overall password length requirements if the rule is enabled
            if (AuthCore.config.passwordRules.length.enabled && (lengthCount < AuthCore.config.passwordRules.length.min || lengthCount > AuthCore.config.passwordRules.length.max))
                return Logger.toUser(false, player.networkHandler, AuthCore.messages.promptUserPasswordLengthIssue, AuthCore.config.passwordRules.length.min, AuthCore.config.passwordRules.length.max);
