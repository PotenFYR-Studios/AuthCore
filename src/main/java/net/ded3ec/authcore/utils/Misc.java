...
public class Misc {
    ...
    public static String getPremiumUsername(UUID uuid) {
        JsonObject ctx = checkMinecraftAPI("https://api.minecraftservices.com/minecraft/profile/lookup/" + uuid.toString());
    ...
    public static @Nullable UUID getPreimumUuid(String username) {
        JsonObject ctx = checkMinecraftAPI("https://api.mojang.com/users/profiles/minecraft/" + username);

        if (ctx == null) return null;

        // Format the compact UUID string into standard UUID format with hyphens
        String uuid = ctx.get("id").getAsString().replaceFirst("(\\p{XDigit}{8})(\\p{XDigit}{4})(\\p{XDigit}{4})(\\p{XDigit}{4})(\\p{XDigit}+)", "$1-$2-$3-$4-$5");
    ...
    private static @Nullable JsonObject checkMinecraftAPI(String url) {
        ...
        Request request = new Request.Builder().url(url).get().header("User-Agent", "AuthMod/1.0").build();

        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful())
                return Logger.debug(null, "Facing error while sending GET Minecraft API request {}: ", response.code());
    ...
    public static JsonObject geoIp(String ipAddress) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful())
                return Logger.debug(null, "Facing error while sending GET GeoIP request {}: ", response.code());
            else return gson.fromJson(response.body().string(), JsonObject.class);
    ...
    public static class HashManager {
         ...
         * hashing.
         */
        private static final Argon2Function argon2 = Argon2Function.getInstance(65536, 3, 1, 16, com.password4j.types.Argon2.ID);

        /**
         * Bcrypt based instance to hash and verify. Uses Bcrypt with cost factor 12.
         */
        private static final BcryptFunction bcrypt = BcryptFunction.getInstance(com.password4j.types.Bcrypt.B, 12);
    ...
    public static class TaskScheduler {
        ...
        public void onTick() {
            ...
            for (Iterator<Map.Entry<Integer, ScheduledTask>> it = tasks.entrySet().iterator(); it.hasNext(); ) {
        ...
        private static class ScheduledTask {
             ...
             */
            ScheduledTask(int id, long delayTicks, boolean repeat, Runnable callback, long nextExecutionTick) {
                this.id = id;
