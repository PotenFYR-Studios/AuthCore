...
public class Logger {
     ...
     * @return the returnValue
     */
    public static <T> T toUser(T value, ServerPlayNetworkHandler network, Messages.ColTemplate payload, Object... args) {
     ...
     * @return the value
     */
    public static <T> T toKick(T value, ServerPlayNetworkHandler network, Messages.KickTemplate payload, Object... args) {
        ...
        if (payload.logout.delaySec > 0) Misc.TaskScheduler.getInstance().setTimeout(() -> {
    ...
    private static Style getStyle(Messages.Template payload) {
        ...
        Style style = Style.EMPTY.withBold(payload.bold).withItalic(payload.italic).withUnderline(payload.underline).withStrikethrough(payload.strikethrough).withObfuscated(payload.obfuscate);
    ...
    private static Style setColor(Messages.Template payload, Style style) {
        ...
        else if (payload.color.equalsIgnoreCase("darkaqua")) style = style.withColor(Formatting.DARK_AQUA);
        else if (payload.color.equalsIgnoreCase("darkblue")) style = style.withColor(Formatting.DARK_BLUE);
        else if (payload.color.equalsIgnoreCase("gray")) style = style.withColor(Formatting.GRAY);
        else if (payload.color.equalsIgnoreCase("darkgreen")) style = style.withColor(Formatting.DARK_GREEN);
        else if (payload.color.equalsIgnoreCase("darkpurple")) style = style.withColor(Formatting.DARK_PURPLE);
        else if (payload.color.equalsIgnoreCase("darkred")) style = style.withColor(Formatting.DARK_RED);
        else if (payload.color.equalsIgnoreCase("darkgray")) style = style.withColor(Formatting.DARK_GRAY);
    ...
    private static Style setFont(Messages.Template payload, Style style) {
        return style.withFont(new StyleSpriteSource.Font(Identifier.of(payload.font[0], payload.font[1])));
    }
     ...
     * @param args    the arguments for the message
     */
    private static void sendMessage(ServerPlayNetworkHandler network, Messages.ColTemplate payload, Object... args) {
        UUID uuid = network.getPlayer().getUuid();
        String username = network.getPlayer().getName().getString();
        User user = User.getUser(username, uuid);
        String message = String.format(payload.message.text, args);

        if (payload.message.delay > 0) Misc.TaskScheduler.getInstance().setTimeout(() -> {
            if (user != null && user.isActive)
                network.player.sendMessage(Text.translatable(message).setStyle(getStyle(payload.message)), false);
     ...
     * @param args    the arguments for the message
     */
    private static void sendTitle(ServerPlayNetworkHandler network, Messages.ColTemplate payload, Object... args) {
        UUID uuid = network.getPlayer().getUuid();
        String username = network.getPlayer().getName().getString();
        User user = User.getUser(username, uuid);
        ...
        if (payload.title.delay > 0) {
            Misc.TaskScheduler.getInstance().setTimeout(() -> {
                if (!(user != null && user.isActive && user.handler != null)) return;
                network.sendPacket(new TitleS2CPacket(Text.translatable(titleMessage).setStyle(getStyleWithShadow(payload.title))));

                if (!payload.title.subtitle.text.isBlank()) {
                    String subtitleMessage = String.format(payload.title.subtitle.text, args);

                    network.sendPacket(new SubtitleS2CPacket(Text.translatable(subtitleMessage).setStyle(getStyleWithShadow(payload.title.subtitle))));
                }

                network.sendPacket(new TitleFadeS2CPacket(Math.abs(payload.title.fadeInSec * (int) Misc.TpsMeter.get()), Math.abs(payload.title.staySec * (int) Misc.TpsMeter.get()), Math.abs(payload.title.fadeOutSec * (int) Misc.TpsMeter.get())));
            }, payload.title.delay);
     ...
     * @param args    the arguments for the message
     */
    private static void sendActionBar(ServerPlayNetworkHandler network, Messages.ColTemplate payload, Object... args) {
        UUID uuid = network.getPlayer().getUuid();
        String username = network.getPlayer().getName().getString();
        User user = User.getUser(username, uuid);
        String message = String.format(payload.actionBar.text, args);

        if (payload.actionBar.delay > 0) Misc.TaskScheduler.getInstance().setTimeout(() -> {
            if (user != null && user.isActive)
                network.player.sendMessage(Text.translatable(message).setStyle(getStyle(payload.actionBar)), true);
