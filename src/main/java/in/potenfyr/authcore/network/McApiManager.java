...
public final class McApiManager {
  ...
  private static final java.util.concurrent.Semaphore LOOKUPS = new java.util.concurrent.Semaphore(4);

  /** Batch GeoIP: collects IPs over a window, deduplicates, resolves in parallel. */
  private static final java.util.Set<String> BATCH_QUEUE = ConcurrentHashMap.newKeySet();
  private static volatile long BATCH_FLUSH_AT = 0L;
  private static final long BATCH_WINDOW_MS = 500L; // collect IPs for 500ms before flushing
  private static final java.util.concurrent.ScheduledExecutorService BATCH_SCHEDULER =
      java.util.concurrent.Executors.newSingleThreadScheduledExecutor(
          r -> {
            Thread t = new Thread(r, "AuthCore-GeoIP-Batch");
            t.setDaemon(true);
            return t;
          });

  static {
    BATCH_SCHEDULER.scheduleAtFixedRate(
        () -> {
          long now = System.currentTimeMillis();
          if (now < BATCH_FLUSH_AT) return;
          java.util.Set<String> batch = new java.util.concurrent.ConcurrentSkipListSet<>();
          synchronized (BATCH_QUEUE) {
            if (BATCH_QUEUE.isEmpty()) return;
            batch.addAll(BATCH_QUEUE);
            BATCH_QUEUE.clear();
            BATCH_FLUSH_AT = 0L;
          }
          // Fire lookups in parallel off the main thread (bounded by LOOKUPS semaphore).
          for (String ip : batch) {
            if (GEOIP_CACHE.containsKey(ip)) continue; // already resolved
            AuthCoreServer.IO_EXECUTOR.execute(() -> geoIp(ip));
          }
        },
        250,
        250,
  ...
  /** Queues an IP for batched GeoIP resolution (non-blocking, safe on server thread). */
  static void queueGeoIp(String ip) {
    if (ip == null || ip.isBlank() || isPrivateAddress(ip)) return;
    if (GEOIP_CACHE.containsKey(ip)) return; // already cached
    if (IN_FLIGHT.contains(ip)) return; // already being resolved
    synchronized (BATCH_QUEUE) {
      if (BATCH_QUEUE.isEmpty()) BATCH_FLUSH_AT = System.currentTimeMillis() + BATCH_WINDOW_MS;
      BATCH_QUEUE.add(ip);
    }
  }
  ...
  public static @Nullable JsonObject geoIp(String ipAddress) {
    ...
    if (cached != null && !cached.isExpired()) return copy(cached.value);
    if (!IN_FLIGHT.add(ipAddress)) return null;
    if (!LOOKUPS.tryAcquire()) {
      IN_FLIGHT.remove(ipAddress);
      return null;
    }
    try {
    ...
    for (String template : GEOIP_PROVIDERS) {
      try {
        ...
        HttpResponse<String> response =
            HTTP_CLIENT.send(request, GeoIpResponse.boundedBody());
        if (response.statusCode() != 200) continue;

        JsonObject parsed = GeoIpResponse.parse(response.body(), template.equals(IPWHOIS_TEMPLATE));
        if (parsed != null) {
          json = parsed;
          break;
        }
  ...
  public static boolean isPrivateAddress(String address) {
    if (!ProxySupport.isValidIp(address)) return true;
    try {
      InetAddress inet = InetAddress.getByName(address);
      byte[] raw = inet.getAddress();
      boolean uniqueLocal = raw.length == 16 && (raw[0] & 0xfe) == 0xfc;
      boolean carrierNat = raw.length == 4 && (raw[0] & 255) == 100 && (raw[1] & 0xc0) == 64;
      return uniqueLocal || carrierNat || inet.isMulticastAddress() || inet.isLoopbackAddress()
  ...
  public static boolean isBedrockPlayer(UUID uuid) {
    if (!in.potenfyr.authcore.compat.Compat.isModLoaded("floodgate")) return false;
  ...
  public static final class PermissionUtil {
    ...
    private static synchronized boolean isLuckPermsLoaded() {
      ...
      lpLoaded = in.potenfyr.authcore.compat.Compat.isModLoaded("luckperms");
