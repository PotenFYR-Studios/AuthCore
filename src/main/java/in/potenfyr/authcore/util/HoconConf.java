...
public class HoconConf {
      ...
      AuthCoreServer.LOGGER.error(
          ...
          "["
              + in.potenfyr.authcore.util.ErrorCodes.code(
                  in.potenfyr.authcore.util.ErrorCodes.Module.CONFIG, in.potenfyr.authcore.util.ErrorCodes.Kind.IO, 1)
  ...
  public static void initialize() {
    try {
      ...
      in.potenfyr.authcore.util.ConfigMigrator.refreshEnrichedMessageDefaults();

      // Always-on safety normalization (blindness breaks the auth chat).
      in.potenfyr.authcore.util.ConfigMigrator.normalizeBlindness();
      ...
      in.potenfyr.authcore.util.ConfigMigrator.migrate();
      ...
      AuthCoreServer.LOGGER.error(
          ...
          "["
              + in.potenfyr.authcore.util.ErrorCodes.code(
                  in.potenfyr.authcore.util.ErrorCodes.Module.CONFIG, in.potenfyr.authcore.util.ErrorCodes.Kind.PARSE, 1)
  ...
  private static org.spongepowered.configurate.ConfigurationNode buildMergedNode() {
    ...
    for (var entry : SECTION_FILES.entrySet()) {
      ...
      try {
        ...
        // mergeFrom fills missing values; make the section the primary tree, then
        // fill only its missing keys from the legacy block.
        wrapped.node(entry.getKey()).mergeFrom(root.node(entry.getKey()));
        root.node(entry.getKey()).set(wrapped.node(entry.getKey()));
  ...
  public static void loadConfig() {
    try {
      ...
      in.potenfyr.authcore.security.SecurityConfiguration.enforce(AuthCoreServer.config);
    }
  }
  ...
  private static void applyRedisConfigOverrides() {
    ...
    String overrides = in.potenfyr.authcore.network.RedisManager.getConfigOverrides();
