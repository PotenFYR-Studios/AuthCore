package in.potenfyr.authcore.util;
...
public class Logger {
   ...
   *
   * <p>Substitutes slf4j-style {@code {}} placeholders sequentially (and {@code %s}-style
   * specifiers when present) so log lines show REAL values on every runtime. Without the
   * {@code {}} handling, entire blocks such as the startup banner printed literal "{}" on
   * 1.16-1.18-era loaders (which ship log4j but no slf4j) - hiding the version, detected
   * Minecraft version, database type and every security flag the banner is supposed to show.
   */
    ...
    if (args != null && args.length > 0) {
      if (text.contains("{}")) {
        ...
        while (argIndex < args.length) {
          int place = text.indexOf("{}", index);
          if (place < 0) break;
          sb.append(text, index, place).append(args[argIndex++]);
          index = place + 2;
        }
        ...
        try {
          ...
          // keep the raw template - never throw on a bad format string
        }
      }
    }
    System.out.println("[AuthCore/" + level + "] " + text);
  }
  ...
  public <T> T toKick(
    ...
    Component reason = in.potenfyr.authcore.compat.Compat.text(message).setStyle(getStyle(payload.logout));
  ...
  private Style getStyleWithShadow(Messages.Template payload) {
    ...
    return in.potenfyr.authcore.compat.Compat.applyShadow(style, payload.shadow, payload.shadowStrength);
  }
  ...
  private Style getStyle(Messages.Template payload) {
    ...
    Style style =
        in.potenfyr.authcore.compat.Compat.applyStyleFlags(
            Style.EMPTY,
            payload.bold,
            payload.italic,
            payload.underline,
  ...
  private void sendMessage(
    ...
    net.minecraft.network.chat.Component component =
        in.potenfyr.authcore.compat.Compat.withClickCommand(
            in.potenfyr.authcore.compat.Compat.text(message).setStyle(style),
  ...
  private static void sendChatMessage(
    ...
    in.potenfyr.authcore.compat.Compat.sendSystemMessage(player, component, false);
  }
  ...
  private void sendTitle(
    ...
    Runnable sendPackets =
        () -> {
          ...
          boolean sent =
              in.potenfyr.authcore.compat.Compat.sendTitle(
                  connection,
                  in.potenfyr.authcore.compat.Compat.text(titleMessage)
                      .setStyle(getStyleWithShadow(payload.title)),
                  payload.title.subtitle != null && !payload.title.subtitle.text.isBlank()
                      ? in.potenfyr.authcore.compat.Compat.text(format(payload.title.subtitle.text, args))
          ...
          if (!sent) {
            try {
              in.potenfyr.authcore.compat.Compat.sendSystemMessage(
                  connection.player,
                  in.potenfyr.authcore.compat.Compat.text(titleMessage)
                      .setStyle(getStyleWithShadow(payload.title)),
                  false);
  ...
  private void sendActionBar(
    ...
    if (payload.actionBar.delay > 0)
          ...
          .setTimeout(
              () -> {
                if (user != null && user.isActive && user.connection != null)
                  in.potenfyr.authcore.compat.Compat.sendSystemMessage(
                      connection.player,
                      in.potenfyr.authcore.compat.Compat.text(message)
      ...
      in.potenfyr.authcore.compat.Compat.sendSystemMessage(
          connection.player,
          in.potenfyr.authcore.compat.Compat.text(message).setStyle(getStyleWithShadow(payload.actionBar)),
          true);
  }
}
