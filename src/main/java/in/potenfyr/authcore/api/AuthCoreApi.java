package in.potenfyr.authcore.api;
...
public final class AuthCoreApi {
  ...
  public static boolean kickPlayer(UUID uuid, String reason) {
    ...
    user.connection.disconnect(in.potenfyr.authcore.compat.Compat.text(reason));
    return true;
  }
  ...
  public static boolean sendMessage(UUID uuid, String message) {
    ...
    in.potenfyr.authcore.compat.Compat.sendMessage(user.connection.player, in.potenfyr.authcore.compat.Compat.text(message), false);
    return true;
  }
  ...
  private static @Nullable ServerPlayer playerOf(User user) {
    try {
