...
public final class AuthIntelligence {
  ...
  private static void alert(String event, String message) {
    ...
    in.potenfyr.authcore.network.Webhook.sendEmbed("AuthCore - " + event, message, 0xE74C3C);
  }
  ...
  /** ip -> recent login timestamps (for bot-farm timing-synchronization detection). */
  private static final Map<String, java.util.List<Long>> IP_LOGIN_TIMESTAMPS = new ConcurrentHashMap<>();

  /** Window (ms) for collecting login timestamps per IP. */
  private static final long TIMING_WINDOW_MS = 60_000L; // 1 minute

  /** Minimum logins from same IP in the window to analyze timing. */
  private static final int TIMING_MIN_SAMPLES = 3;

  /** Threshold for coefficient of variation below which logins are considered synchronized. */
  private static final double TIMING_CV_THRESHOLD = 0.15;

  /** Called on every successful login. Detects bot-farm synchronized login timers. */
  public static void recordLoginTiming(String ip, String username) {
    Config.Session.AuthIntelligenceConfig cfg = config();
    if (cfg == null || ip == null || ip.isBlank()) return;

    Config.Session.LoginTimingConfig timingCfg = cfg.loginTiming;
    if (timingCfg == null || !timingCfg.enabled) return;

    long now = System.currentTimeMillis();
    java.util.List<Long> stamps =
        IP_LOGIN_TIMESTAMPS.computeIfAbsent(ip, k -> new java.util.ArrayList<>());

    synchronized (stamps) {
      stamps.add(now);
      // Prune stale entries
      long cutoff = now - TIMING_WINDOW_MS;
      int before = stamps.size();
      stamps.removeIf(t -> t < cutoff);
      if (stamps.size() < TIMING_MIN_SAMPLES) return;

      // Compute coefficient of variation of intervals between consecutive logins.
      // Bots use synchronized timers → near-zero variance. Humans have natural variation.
      if (stamps.size() < TIMING_MIN_SAMPLES) return;
      double[] intervals = new double[stamps.size() - 1];
      double sum = 0, sumSq = 0;
      int valid = 0;
      for (int i = 1; i < stamps.size(); i++) {
        double d = stamps.get(i) - stamps.get(i - 1);
        if (d <= 0 || d > TIMING_WINDOW_MS) continue; // skip outliers
        intervals[valid++] = d;
        sum += d;
        sumSq += d * d;
      }
      if (valid < TIMING_MIN_SAMPLES - 1) return;
      double mean = sum / valid;
      if (mean < 100) return; // too fast to measure
      ...
      if (cv < TIMING_CV_THRESHOLD) {
        alert(
            "LOGIN_TIMING_SYNC",
            "**" + username + "** from `" + ip + "` had "
                + stamps.size()
                + " logins with CV="
                + String.format("%.3f", cv)
                + " in "
                + (TIMING_WINDOW_MS / 1000)
                + "s - bot-farm synchronized timers detected.");
      }
    }

    // Keep attacker-controlled IP cardinality bounded.
    if (IP_LOGIN_TIMESTAMPS.size() > 4096) {
      long cutoff = now - TIMING_WINDOW_MS;
      IP_LOGIN_TIMESTAMPS.entrySet().removeIf(e -> {
        java.util.List<Long> l = e.getValue();
        synchronized (l) {
          l.removeIf(t -> t < cutoff);
          return l.isEmpty();
        }
      });
    }
  ...
  public static void recordPasswordChange(User user, String ip) {
    ...
    AuthCoreServer.LOGGER.debug(
        ...
        user.username,
        ip,
        lastLogin <= 0 ? "n/a" : in.potenfyr.authcore.util.TimeManager.toDuration(System.currentTimeMillis() - lastLogin) + " ago",
