...
public final class ActionCaptcha {
  ...
  public static void onLogin(ServerPlayer player) {
    ...
    TaskScheduler.getInstance()
        .setTimeout(
            () -> {
              ServerPlayer current = in.potenfyr.authcore.compat.Compat.getPlayerByUuid(uuid);
              if (current == null) return;
              User currentUser = User.getUser(uuid);
              if (currentUser == null) return;
              evaluate(current, currentUser);
            },
  ...
  private static void evaluate(ServerPlayer player, User user) {
    ...
    if (cfg.disableWhenTpsBelow > 0
        && in.potenfyr.authcore.util.TpsManager.get() < cfg.disableWhenTpsBelow) {
      AuthCoreServer.LOGGER.debug(
          false,
          "{} | verification skipped - TPS {} below {}", user.username, in.potenfyr.authcore.util.TpsManager.get(), cfg.disableWhenTpsBelow);
      return;
    }
    ...
    // 2b. Discrete-interaction timing: bots often issue inventory/chat actions at regular
    // intervals, while humans have natural variation. Movement is deliberately excluded
    // because vanilla clients emit it on a regular server tick.
    if (p != null) {
      double regularity = p.timingRegularityScore();
      int regularityScore = (int) Math.round(regularity * 15);
      if (regularityScore > 0) {
        score += regularityScore;
        breakdown.append("timing+").append(regularityScore).append(' ');
      } else {
        breakdown.append("timing0 ");
      }
    }
  ...
  private static void send(ServerPlayer player, String text, ChatFormatting color) {
    try {
      ...
      net.minecraft.network.chat.Component component =
          in.potenfyr.authcore.compat.Compat.text(text).setStyle(style);
