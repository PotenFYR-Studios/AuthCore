...
public class Config {
  ...
  public in.potenfyr.authcore.models.Config.Database database = new in.potenfyr.authcore.models.Config.Database();
  ...
  public static class ClientGuardConfig {
    ...
    @Comment(
        """
                Look-pattern bot detection: analyze camera-movement regularity every N seconds.
                Bots have machine-precision look deltas; humans have natural variation.
                0 = disabled. Default: 15""")
    public int lookPatternCheckIntervalSec = 15;
  ...
  public static class AuthIntelligenceConfig {    @Comment("Password-spraying detection (same password tried on many accounts). Default: true")
    ...
    @Comment("Login-timing-synchronization detection (bot farms use synchronized timers). Default: true")
    public LoginTimingConfig loginTiming = new LoginTimingConfig();
  }
  ...
  @ConfigSerializable
  public static class LoginTimingConfig {

    @Comment("Enable login-timing-synchronization detection (bot farms use synchronized timers). Default: true")
    public boolean enabled = true;

    @Comment("Window (seconds) to collect login timestamps per IP. Default: 60")
    public int windowSec = 60;

    @Comment("Minimum distinct logins from the same IP in the window to analyze. Default: 3")
    public int minSamples = 3;

    @Comment("Coefficient-of-variation threshold below which logins are considered synchronized. Default: 0.15")
    public double cvThreshold = 0.15;
  }
