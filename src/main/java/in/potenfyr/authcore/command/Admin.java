package in.potenfyr.authcore.command;
...
public class Admin {
  ...
  private static int backupDatabaseCommand(CommandSourceStack source) {
    try {
      ServerPlayer player = in.potenfyr.authcore.compat.Compat.sourcePlayer(source);
      ...
      in.potenfyr.authcore.security.SecurityLog.log("DB_BACKUP", "Created " + backup);
  ...
  private static int exportUsersCommand(CommandSourceStack source) {
    try {
      ...
      in.potenfyr.authcore.security.SecurityLog.log("DB_EXPORT", "Exported " + list.size() + " users to " + out);
  ...
  private static int maintenanceCommand(CommandSourceStack source, boolean enabled) {
    try {
      ...
      in.potenfyr.authcore.util.HoconConf.saveConfig();
  ...
  private static int validateConfigCommand(CommandSourceStack source) {
    try {
      ...
      // Webhook
      if (in.potenfyr.authcore.network.Webhook.isEnabled()) {
      ...
      net.minecraft.server.MinecraftServer srcServer = in.potenfyr.authcore.compat.Compat.getSourceServer(source);
      ServerPlayer player = in.potenfyr.authcore.compat.Compat.sourcePlayer(source);
      if (srcServer == null && player != null) {
        srcServer = in.potenfyr.authcore.compat.Compat.getServer(player);
      }
      if (srcServer != null && !AuthCoreServer.isServerDetected()) {
        AuthCoreServer.detectServerOnlineMode(in.potenfyr.authcore.compat.Compat.serverUsesAuthentication(srcServer));
      }
  ...
  private static int compatReportCommand(CommandSourceStack source) {
    try {
      ...
      report.append("  Minecraft     : ").append(in.potenfyr.authcore.compat.Compat.getGameVersion()).append('\n');
      report.append("  Loader        : ").append(in.potenfyr.authcore.compat.Compat.getLoaderVersion()).append('\n');
      ...
      String integrations = in.potenfyr.authcore.integration.ModIntegrations.status();
      for (String line : integrations.split("\n"))
        if (!line.isBlank()) report.append("  ").append(line).append('\n');

      report.append("  Hooks         : ").append("commands ").append(in.potenfyr.authcore.util.FabricHooks.class.getSimpleName()).append(" / mixins / fabric-api reflectively\n");
  ...
  private static int importAuthMeCommand(CommandSourceStack source, String dbFilePath) {
    try {
      ...
      in.potenfyr.authcore.util.AuthMeImporter.ImportResult result =
          in.potenfyr.authcore.util.AuthMeImporter.importSqlite(path);
  ...
  private static int reloadCommand(CommandSourceStack source) {
    try {
      ...
      // Rotate the per-server companion attestation key on reload (invalidates all pending
      // challenges - a live key change forces fresh re-attestation).
  ...
  private static int deletePlayerCommand(CommandSourceStack source, ServerPlayer player) {
    try {
      ServerPlayer sourcePlayer = in.potenfyr.authcore.compat.Compat.sourcePlayer(source);
      ...
      if (user == null && player != null)
        ...
        return AuthCoreServer.LOGGER.info(
            ...
            user.isAuthenticated.get()
                ...
                : "False",
            in.potenfyr.authcore.security.ClientGuard.profile(user.uuid) != null
                ? "tracked (brand="
                    + in.potenfyr.authcore.security.ClientGuard.profile(user.uuid).brand
                    + ", settings="
                    + in.potenfyr.authcore.security.ClientGuard.profile(user.uuid).settingsSeen
                    + ", companion="
                    + in.potenfyr.authcore.security.ClientGuard.profile(user.uuid).claimsCompanion
                    + "/"
                    + in.potenfyr.authcore.security.ClientGuard.profile(user.uuid).attestationOk
                    + ")"
                : "untracked (offline)",
            in.potenfyr.authcore.security.ClientGuard.profile(user.uuid) != null
                ? in.potenfyr.authcore.security.ClientGuard.profile(user.uuid).risk
                : 0,
            in.potenfyr.authcore.security.ClientGuard.profile(user.uuid) != null
                ? in.potenfyr.authcore.security.ClientGuard.profile(user.uuid).signals
                : "[]");
