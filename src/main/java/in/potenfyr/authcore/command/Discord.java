...
public class Discord {
  ...
  private static int linkCommand(CommandSourceStack source) {
    try {
      ...
      if (!in.potenfyr.authcore.security.RateLimiter.tryAcquire("discordlink:" + player.getUUID(), 1, 60_000L))
        return AuthCoreServer.LOGGER.toUser(
  ...
  private static int unlinkCommand(CommandSourceStack source) {
    try {
      ...
      in.potenfyr.authcore.security.SecurityLog.log("DISCORD_UNLINK", user.username + " unlinked Discord");
