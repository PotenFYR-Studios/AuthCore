...
public final class McApiManager {
  ...
  public static final class PermissionUtil {
     ...
     */
    public static ServerPlayerEntity resolvePlayer(Object ctx) {
      return net.ded3ec.compat.Compat.sourcePlayer(ctx);
    }
    ...
    public static boolean hasForSource(Object ctx, String node, int level) {
      ServerPlayerEntity player = resolvePlayer(ctx);
    ...
    public static boolean hasLevel(ServerPlayerEntity player, int level) {
      ...
      try {
        return (Boolean)
            ServerPlayerEntity.class.getMethod("hasPermissionLevel", int.class).invoke(player, level);
      } catch (ReflectiveOperationException err) {
        return false;
      }
    }
