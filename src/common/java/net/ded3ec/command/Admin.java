...
public class Admin {
  ...
  private static int reloadCommand(ServerCommandSource source) {
    try {
      ...
      if (player != null)
        return AuthCoreServer.LOGGER.toUser(
            1, player.networkHandler, AuthCoreServer.messages.promptAdminReloadedConfiguration);
      else
        return AuthCoreServer.LOGGER.info(
  ...
  private static int deletePlayerCommand(ServerCommandSource source, ServerPlayerEntity player) {
    try {
      ServerPlayerEntity sourcePlayer = Compat.sourcePlayer(source);
  ...
  private static int listPlayersCommand(ServerCommandSource source) {
    try {
      ...
      if (player != null)
        return AuthCoreServer.LOGGER.toUser(
            1, player.networkHandler, AuthCoreServer.messages.promptAdminListOfPlayers, "Players");
  ...
  private static int destroyUserSessionCommand(
    ...
    try {
      ...
      if (!user.isActive && sourcePlayer != null)
        return AuthCoreServer.LOGGER.toUser(
            0, sourcePlayer.networkHandler, AuthCoreServer.messages.promptAdminUserIsNotActive);
  ...
  private static int WhoIsUserByUsernameCommand(
    ...
    try {
      ...
      if (user == null && player != null)
        return AuthCoreServer.LOGGER.toUser(
            1, player.networkHandler, AuthCoreServer.messages.promptAdminUserNotFound, uniqueId);
      else if (user == null)
        return AuthCoreServer.LOGGER.info(
            1,
        ...
        return AuthCoreServer.LOGGER.toUser(
            1, player.networkHandler, AuthCoreServer.messages.promptAdminWhoIsUser, user.username);
      else
        return AuthCoreServer.LOGGER.info(
            1,
            "Information about '{}':\nUUID: {}\nPlatform: {}\nMode: {}\nIP-Address: {}\nStatus: {}\nOffline Registered: {}\nCountry: {}\nuser Created (date): {}\nAuthenticated: {}",
